from sklearn.feature_extraction.text import CountVectorizer #create a document term matrix using CountVectorizer 
...
data_vector = vectorizer.fit_transform(data_clean.Transcripts) #fit count vectorizor to the clean transcipt data

# Convert it to an array and label all the columns
data_tokenized = pd.DataFrame(data_vector.toarray(), columns=vectorizer.get_feature_names())
