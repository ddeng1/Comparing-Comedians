...
# Change the values of the transcripts from being a list to a string of text
...
data = pd.DataFrame.from_dict(data_combined,
                              orient='index', columns=["Transcripts"])
...
def clean_text(text):
    ...
    # Remove additional quotation marks in some transcript
    new_text = re.sub('[‘’“”…]', '', new_text)
    # remove newline text, \n
    new_text = re.sub('\n', '', new_text)
    return new_text
...
data_clean = pd.DataFrame(data.Transcripts.apply(clean_text))

# Add comedians full names
full_names = ["Trevor Noah", "Dave Chappelle", "Joe Rogan",
